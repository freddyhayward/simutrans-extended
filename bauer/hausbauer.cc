			...
			if(gr) {
				...
				if(  gb_part  &&  gb_part->get_tile()==bdsc->get_tile(layout, k.x, k.y)  ) {
					...
					if(gr->get_typ()==grund_t::fundament) {
						...
						sint8 new_hgt;
						const slope_t new_slope = welt->recalc_natural_slope(newk,new_hgt);
						// test for ground at new height
						const grund_t *gr2 = welt->lookup(koord3d(newk,new_hgt));
						if(  (gr2==NULL  ||  gr2==gr) && !new_slope.is_flat()  ) {
						...
						if(  gr2  &&  gr2!=gr  ) {
							...
							welt->access(newk)->kartenboden_setzen( new boden_t(gr->get_pos(), slope_t() ) );
							ground_recalc = false;
						}
						else if(  new_hgt <= welt->get_water_hgt(newk)  && !new_slope.is_flat()  ) {
...
//DBG_DEBUG("hausbauer_t::build()","get_tile() at %i,%i",k.x,k.y);
			...
			if(!gr->is_water() && desc->get_type() != building_desc_t::dock && desc->get_type() != building_desc_t::flat_dock)
			{
				...
				needs_ground_recalc |= !gr->get_grund_hang().is_flat();
				...
				if (dim.x == 1 && dim.y == 1) {
					...
					if(  front_side_neighbor != koord(0,0)  ) {
						...
						if(  !front_gr || (!front_gr->get_weg_hang().is_flat())  ) {
							...
							if(  other_front_side_neighbor != koord(0,0)  ) {
								...
								if (other_front_gr && (other_front_gr->get_weg_hang().is_flat())  ) {
