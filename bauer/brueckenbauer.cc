...
bool bridge_builder_t::is_blocked(koord3d pos, ribi_t::ribi check_ribi, player_t* player, const char *&error_msg)
{
	...
	for(int dz = -clearance -2; dz <= clearance; dz++) {
		...
		if (dz != 0 && (gr2 = welt->lookup(pos + koord3d(0,0,dz)))) {
			...
			if (dz < -clearance) {
				if (dz + slope.max_diff() + gr2->get_weg_yoff() / TILE_HEIGHT_STEP < -clearance ) {
					// too far below
					continue;
				}
			}

			if (dz + slope.max_diff() == 0 && gr2->ist_karten_boden() && ribi_type(gr2->get_grund_hang()) == check_ribi) {
...
koord3d bridge_builder_t::find_end_pos(player_t *player, koord3d pos, const koord zv, const bridge_desc_t *desc, const char *&error_msg, sint8 &bridge_height, bool ai_bridge, uint32 min_length, bool high_bridge )
{
	...
	if(slope.max_diff() == 2 && !desc->has_double_start()  ) {
	...
	do {
		...
		const sint16 hang_height = gr->get_hoehe() + end_slope.max_diff() + gr->get_weg_yoff() / TILE_HEIGHT_STEP;
		...
		if(length >= min_length && end_slope.allows_way() &&
           (gr->get_typ()==grund_t::boden || gr->get_typ()==grund_t::tunnelboden)) {
			bool finish = false;
			if(height_okay(hang_height) && end_slope.is_flat() &&
               (hang_height == max_height || ai_bridge || min_length)) {
				...
				if(end_slope.max_diff() == 2 && !desc->has_double_start()  ) {
		...
		const slope_t end_ground_slope = gr->get_grund_hang();
		const sint16 hang_ground_height = gr->get_hoehe() + end_ground_slope.max_diff();
...
void bridge_builder_t::build_bridge(player_t *player, const koord3d start, const koord3d end, koord zv, sint8 bridge_height, const bridge_desc_t *desc, const way_desc_t *way_desc, overtaking_mode_t overtaking_mode)
{
	...
	const slope_t slope = start_gr->get_weg_hang();
	...
	if (!slope.is_flat() || bridge_height != 0) {
		// needs a ramp to start on ground
		add_height = !slope.is_flat() ? slope.max_diff() : bridge_height;
		build_ramp( player, start, ribi, !slope.is_flat() ? slope_t() : slope_t(slope_type(zv).get_value() * add_height), desc, way_desc, overtaking_mode, true );
		if(  desc->get_waytype() != powerline_wt  ) {
		...
		if(  desc->get_waytype() == powerline_wt  ) {
			...
			const slope_t hang = start_gr ? start_gr->get_weg_hang() : slope_t();
			if(!hang.is_flat())
			{
	...
	while(  pos.get_2d() != end.get_2d()  ) {
		brueckenboden_t *bruecke = new brueckenboden_t( pos, slope_t(), slope_t() );
		...
		bruecke_t *br = new bruecke_t(bruecke->get_pos(), player, desc, desc->get_straight(ribi, height - gr->get_grund_hang().max_diff()));
		bruecke->obj_add(br);
		bruecke->calc_image();
		br->finish_rd();
...
//DBG_MESSAGE("bool bridge_builder_t::build_bridge()","h1=%i, h2=%i",pos.z,gr->get_pos().z);
	...
	if(  need_auffahrt  ) {
		...
		build_ramp(player, end, ribi_type(-zv), !gr->get_weg_hang().is_flat()?slope_t():slope_t(slope_type(-zv).get_value()*(pos.z-end.z)), desc, way_desc, overtaking_mode, false);
	}
	else {
...
void bridge_builder_t::build_ramp(player_t* player, koord3d end, ribi_t::ribi ribi_neu, slope_t weg_hang, const bridge_desc_t* desc, const way_desc_t *way_desc, overtaking_mode_t overtaking_mode, bool beginning)
{
	assert(weg_hang.get_value() <= slope_t::max_number);

	grund_t *alter_boden = welt->lookup(end);
	brueckenboden_t *bruecke;
	slope_t grund_hang = alter_boden->get_grund_hang();
...
const char *bridge_builder_t::remove(player_t *player, koord3d pos_start, waytype_t wegtyp)
{
	...
	do {
		...
		if(pos != pos_start && from->ist_karten_boden()) {
			...
			if(!from->get_grund_hang().is_flat()) {
				zv = -koord(from->get_grund_hang());
			}
			else if(!from->get_weg_hang().is_flat()) {
	...
	while(  !end_list.empty()  ) {
		...
		if(wegtyp==powerline_wt) {
			...
			if(  weg  ) {
				...
				weg->set_ribi( ribi );
				if(gr->get_weg_hang().max_diff() >= 2 && !weg->get_desc()->has_double_slopes()  ) {
