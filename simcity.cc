...
bool stadt_t::bewerte_loc(const koord pos, const rule_t &regel, int rotation)
{
	...
	FOR(vector_tpl<rule_entry_t>, const& r, regel.rule) {
		...
		switch (r.flag) {
			...
			case 'U':
				// unbuildable for road
				if (!gr->get_grund_hang().allows_way()) return false;
				break;
			case 'u':
				// road may be buildable
				if (gr->get_grund_hang().allows_way()) return false;
...
class monument_placefinder_t : public placefinder_t {
	public:
		...
		virtual bool is_tile_ok(koord pos, koord d, climate_bits cl, uint16 allowed_regions) const
		{
			...
			if (gr->get_typ() == grund_t::fundament) {
				sint8 new_hgt;
				const slope_t new_slope = welt->recalc_natural_slope(gr->get_pos().get_2d(),new_hgt);

				if (!new_slope.is_flat()) {
					return false;
				}
			}
...
class townhall_placefinder_t : public placefinder_t {
	public:
		...
		virtual bool is_tile_ok(koord pos, koord d, climate_bits cl, uint16 allowed_regions) const
		{
			...
			if (gr == NULL  || !gr->get_grund_hang().is_flat()) return false;
				...
				for (k.x = 0; k.x < groesse_alt.x; k.x++) {
					for (k.y = 0; k.y < groesse_alt.y; k.y++) {
						...
						if (gr  &&  gr->ist_natur() &&  gr->kann_alle_obj_entfernen(NULL) == NULL  &&
							  (gr->get_grund_hang().is_flat() || welt->lookup(koord3d(k, welt->max_hgt(k))) == NULL  ) ) {
		...
		for(uint8 h=(layout_origin&1)?dim_origin.x:dim_origin.y; h<=LEN_LIM; h++) {
			...
			for(uint8 x=0; x<w; x++) {
				...
				for(uint8 y=0; y<h; y++) {
					...
					sint8 tile_height;
					const slope_t hang = welt->recalc_natural_slope(p, tile_height);
					if(!hang.is_flat()) {
						tile_height ++;
					}
...
void stadt_t::build_city_building(const koord k, bool new_town, bool map_generation)
{
	...
	if(!gr->get_grund_hang().is_flat() && welt->lookup(koord3d(k, welt->max_hgt(k))) != NULL  ) {
		return;
	}
...
bool stadt_t::build_road(const koord k, player_t* player_, bool forced, bool map_generation)
{
	...
	slope_t slope = bd->get_grund_hang();
	if (!slope.allows_way()) {
	...
	ribi_t::ribi allowed_dir = (!bd->get_grund_hang().is_flat() ? ribi_t::doubles(ribi_type(bd->get_weg_hang())) : (ribi_t::ribi)ribi_t::all);
		...
		if (ribi_t::nsew[r] & allowed_dir) {
			...
			if(bd->get_neighbour(bd2, invalid_wt, ribi_t::nsew[r])) {
				if(bd2->get_typ()==grund_t::fundament  ||  bd2->get_typ()==grund_t::wasser) {
					...
					&&  (bd2->get_grund_hang().is_flat() ? ribi_t::nsew[r] != ribi_type(bd2->get_weg_hang())
	...
	if (connection_roads != ribi_t::none || forced) {
		...
		if(ribi_t::is_single(connection_roads)) {
			...
			if(  bd_next &&
				...
				if (err && *err != 0 && strcmp(err, "Bridge is too long for this type!\n") != 0 && !bd->get_weg_hang().is_flat()) {
					slope_t old_slope = bd->get_grund_hang();
					sint8 h_diff = old_slope.max_diff();
					// raise up the tile
					bd->set_grund_hang(slope_t());
